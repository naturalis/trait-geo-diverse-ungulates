...
```{r domestication}
traits.means.file <- sprintf('%s/results/OMI/standardized.averages.csv', REPO_HOME)
...
pglm.formula.tmpl <- 'IsDomesticated ~ bio3 + bio14 + current_10arcmin_embergerQ + bio15 + bio2 + %s'
...
```{r plot3d}
f <- IsDomesticated ~ bio3 + bio14 + current_10arcmin_embergerQ
r <- phyloglm(f, traits.means.tree, tree, method = "logistic_MPLE", btol = 30)
summary(r)


p3d <- as.matrix(dplyr::select(traits.means.tree,bio3,bio14,current_10arcmin_embergerQ))
...
```{r trace}
# load niche tree
niche.tree.file <- sprintf('%s/results/maxent/dendrogram.tree',REPO_HOME)
niche.tree <- read.tree(niche.tree.file)
...
for ( i in 1:length(layers.names) ) {
  ...
  layer.file <- sprintf('%s/results/maxent/%s_contMap.pdf',REPO_HOME,layer.name)
