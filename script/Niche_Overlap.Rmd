...
output_AUC <- data.frame(matrix(ncol = 3, nrow = length(taxa)))
colnames(output_AUC) <- c("taxon","trainingAUC","confidence.interval")
...
for (i in 1: 54) {
  ...
  if (!file.exists(valid.model.file) && !file.exists(invalid.model.file)) {
    ...
    AUCvalues<- nullModel_adjusted(visited_areas, modelenvironment, polygonextent, occurence_samples, rep = 100)
    ...
    # save AUC values 
     output_AUC[i,] <- c(name_underscore, trainingAUC, confidence.interval)
...
for (i in 1:length(list_species_model_high_accuracy)) {
  ...
  if (file.exists(valid.model.prediction)) {
    ...
    # create an extent to select the limitations of the predictions 
