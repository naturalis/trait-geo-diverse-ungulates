...
for (i in 1:length(files_list)) {
  ...
  # open files for 5 degrees or 10 degrees
...
When calculating the species distribution models a dataframe containing all the occurence points is needed and created below. 
```{r combine_occurences}

# create an emtpy dataframe 
empty.df = data.frame(matrix(vector(), 0, 4,
                dimnames=list(c(), c("gbif_id", "taxon_name", "decimal_latitude", "decimal_longitude"))),
                stringsAsFactors=F)
# loop to add the occurence data to the empty dataframe
for (i in 1:length(taxa)) {
  taxon<- taxa[i]
 load.csv <- sprintf("%s/data/filtered/%s.csv", REPO_HOME, taxon)
 csv <- read.csv(load.csv, header = T)
 empty.df<- rbind(empty.df, csv)
}
```
...
for (i in 1:length(taxa)) {
  ...
  if (!file.exists(occ.map.file)) {
    ...
    plot(
      ...
      col = "grey",
      main = sprintf("Filtered occurrences for %s", taxon_name)
    )
...
for (i in 1:length(taxa)) {
  ...
  if (!file.exists(image.name)) {
    ...
    ubio_name <- rphylopic::name_search(text = taxon_name, options = "namebankID")[[1]]
    image_search <- rphylopic::name_images(ubio_name$uid[1])
    image <- rphylopic::image_data(image_search$same[1], size = "128")
    rphylopic::save_png(image[[1]], image.name)
  }
}
```
...
for (i in 1:length(taxa)) {
  ...
  if (!file.exists(valid.model.file) && !file.exists(invalid.model.file)) {
    ...
    AUCvalues<- nullModel_adjusted(visited_areas, modelenvironment, occurence_samples, rep = 3)
    ...
    Validation_species <- dismo::evaluate(
      p = species_model[[3]], # all the occurence datapoints 
