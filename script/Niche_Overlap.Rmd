...
characteristics. Climatic information about the present was subtracted from the widely used Bioclim 
dataset which includes 19 bioclimatic datasets. The datasets contain information such as precipitation 
...
```{r add_layers}
# here we make a temporary file name with the .tif extension. portable between mac and windows
temp_name <- tempfile(pattern = "file", tmpdir = tempdir(), fileext = ".tif")

# with the ?dl=1 argument the file is downloaded directly and stored as the temporary file
download.file("https://www.dropbox.com/s/nlfe349hykg9pfs/Totalstack.tif?dl=1", temp_name, quiet = T)

# here we complete the stacking
soilproperties <- stack(temp_name)
currentEnv <- stack(soilproperties, currentEnv1)

# now remove the temp file and the temporary variable symbols
unlink(temp_name)
rm(temp_name)
rm(currentEnv1)
```
...
line below downloads a list of the species of interest from the 
[github data folder](https://github.com/naturalis/trait-geo-diverse-ungulates) data folder, 
originally obtained via GBIF. The data contains the taxon name, taxon id and the longitude and 
latitude, as well as the identifier of the original occurrence record. The latter is nearly
always an integer (meaning it's a GBIF occurrence ID), though in rare cases it might be a
URL, e.g. of an iNaturalist observation.

```{r load_taxa}
t <- read.table("https://github.com/naturalis/trait-geo-diverse-ungulates/raw/master/data/filtered/taxa.txt")
```

Below an example of how to plot these occurrence datasets is given.

```{r plot_points}
# create the URL for the file whose occurrences you want to plot, then load that as CSV
file <- paste("https://github.com/naturalis/trait-geo-diverse-ungulates/raw/master/data/filtered/", t[5,1], sep = "")
occ <- read.csv(file, header = T)
lon <- occ$decimal_longitude
lat <- occ$decimal_latitude

# plot a simple world map
data(wrld_simpl)
plot(wrld_simpl, axes=TRUE, xlim=c(min(lon)-5,max(lon)+5), ylim=c(min(lat)-5,max(lat)+5), col="grey", main=t[5,1])
