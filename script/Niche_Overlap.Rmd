...
```{r globals}
# the base URL of the location of our data
DATA_URL <- "https://github.com/naturalis/trait-geo-diverse-ungulates/raw/master/data/filtered/"
...
```{r load_taxa}
t <- read.table(paste(DATA_URL, "taxa.txt", sep = ""))
```
...
for (i in 1:10) {
  ...
  file <- paste(DATA_URL, t[i,1], sep = "")
  species_occurence <- read.csv(file, header = T)
  ...
  if (Validation_species@auc > 0.7) {
    list_species_model_high_accuracy[[name]] <- species_model[[1]]
  } 
  else { 
    list_species_model_low_accuracy[[name]] <- species_model[[1]]
  }
}
```
