...
rm(soilproperties,totalstack.tif,currentEnv1)
```
...
for (i in 1:length(taxa)) {
  ...
  if(!file.exists(valid.model.file) && !file.exists(invalid.model.file)) {
    ...
    if (Validation_species@auc > 0.7) {
      ...
      save(species_model, file = valid.model.file)
    } 
    else { 
      list_species_model_low_accuracy[[name]] <- species_model[[1]]
      save(species_model, file = invalid.model.file)
    }
