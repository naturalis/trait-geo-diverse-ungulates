...
for ( i in 1:length(taxa) ) {
taxon <- taxa[i]
image.name <- sprintf('%s/results/per_species/%s/image_taxa.png', REPO_HOME, taxon)

if(!file.exists(image.name)){
taxon_name <- gsub("_", " ", taxa[i])
ubio_name<-name_search(text = taxon_name, options="namebankID")[[1]]
image_search<- name_images(ubio_name$uid[1])

image<- image_data(image_search$same[1] , size="128")

save_png(image[[1]], image.name) 
}
...
for (i in 1:length(taxa)) {
  ...
  if(!file.exists(valid.model.file) && !file.exists(invalid.model.file)) {
    ...
    # Run maxent:
    # - clip environment to ± 5 degrees lat/long relative to the occurrences
    ...
    # to construct an AUC model we need random points drawn from the clipped, variable-selected, environment
    random <- randomPoints(species_model[[2]], 1000)
    Validation_species <- dismo::evaluate(
      p = species_model[[3]],    # 25% sample of input occurrences
      a = random,                # random absences
      x = species_model[[2]],    # the clipped, variable-selected environment
      model = species_model[[1]] # the maxent model
    )

    # populate two stacks, one with the models with a low accuracy <0.7 and one with a high accuracy >0.7
...
valid.model.variable.importance <- sprintf('%s/results/per_species/%s/valid_maxent_variable_importance.pdf', REPO_HOME, name)
valid.model.response.curve <- sprintf('%s/results/per_species/%s/valid_maxent_response_curve.pdf', REPO_HOME, name)
...
for ( i in 1:length(list_species_model_high_accuracy) ) {
  ...
  variable.map.file <- sprintf('%s/results/per_species/%s/valid_maxent_variable_importance.pdf', REPO_HOME, name)
  if ( !file.exists(variable.map.file) ) {
    pdf(file = variable.map.file)
    plot(open_model, main = name2)
    dev.off()
  }
  
  response.map.file <- sprintf('%s/results/per_species/%s/valid_maxent_response_curve.pdf', REPO_HOME, name)
  if ( !file.exists(response.map.file) ) {
    pdf(file = response.map.file)
    response(open_model)
    dev.off()
  }
}
...
for ( i in 1:length(names(prediction_rasters)) ) {
  ...
  prediction.occurence.map.file <- sprintf('%s/results/per_species/%s/prediction_occurence_map.pdf', REPO_HOME, taxon.name)
  ...
  if ( !file.exists(prediction.occurence.map.file) ) {
    ...
    pdf(file = prediction.occurence.map.file)
    plot(prediction_rasters[[i]])
