...
source(paste(REPO_HOME,'/script/phyloglmstep.R',sep=''))
```
...
# cleanup
rm(taxa,tip.idx,tree.file,niche.traits.file)
```
...
# aic=62.335019 from phyloglmstep.R
best.model <- IsDomesticated ~ 1 + bio2 + OrganicCarbon_5min + aridityIndexThornthwaite + minTempWarmest + monthCountByTemp10
pglm.result <- phyloglm(best.model,niche.traits.df,tree,method="logistic_MPLE",btol=100)
...
# relabel columns
readable.names <- c( 'daily temp range', 'dryness', 'year-round warmth' )
names(traits.dom) <- readable.names
names(traits.wild) <- readable.names

# calculate hypervolume
hv.dom <- hypervolume(traits.dom, method = 'svm', name = 'domesticated', svm.nu = 0.001)
hv.wild <- hypervolume(traits.wild, method = 'svm', name = 'wild',  svm.nu = 0.001)
...
```{r deleteme}
m1 <- IsDomesticated ~ bio3 + OrganicCarbon_5min + annualPET + bio10 + monthCountByTemp10 + bio14 + bio4
m1.res <- phyloglm(m1, niche.traits.df, tree, method = "logistic_MPLE", btol = 100, boot = 100)
m1.sum <- summary(m1.res)

pglm.pvalues <- m1.sum$coefficients[,'p.value']
pglm.variables <- names(pglm.pvalues)[2:length(pglm.pvalues)]
pglm.weights <- 1/pglm.pvalues[2:length(pglm.pvalues)]

sig.data <- subset(niche.traits.df, select = pglm.variables)
traits.dist <- daisy(sig.data, metric = "gower", weights = pglm.weights)
traits.tree <- nj(traits.dist)
plot.phylo(traits.tree, type = "fan")
```
