...
forward.model.selection <- function(tree,data,dependent,aic.list = NULL,model.template = NULL,predictors = NULL) {
  ...
  for ( i in 1:length(predictors) ) {
    ...
    result <- phyloglm(myModel, data, tree, method = "logistic_MPLE", btol = 120, boot = 10)
    aics[i] <- result$aic
  }
  ...
  ##############################################################################
  # NOTE!!! NOTE!!! NOTE!!!
  # With the inclusion of the Vicugna (i.e. corrected spelling in the tree!!!)
  # we now experience that adding the second variable first makes the AIC worse,
  # but then the third one makes the best model. Beyond that, we fail to 
  # converge. THERE IS SOMETHING WRONG WITH THE VICUGNA DATA. IT DIFFERS TOO 
  # MUCH FROM THE LLAMA.
...
best.model <- IsDomesticated ~ bio3 + bio14 + embergerQ
