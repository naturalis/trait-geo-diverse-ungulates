...
The PhyloPic web service provides line drawings (silhouettes) for many species.
Here we download such an image for each of our species:
...
for (i in 1:length(taxa)) {
  ...
  if (!file.exists(image.name)) {
    ...
    ubio.name <- rphylopic::name_search(text = taxon.name, options = "namebankID")[[1]]
    image.search <- rphylopic::name_images(ubio.name$uid[1])
    if ( !is.null(image.search$same[1][[1]]) ) {
      image <- rphylopic::image_data(image.search$same[1], size = "128")
...
occurence points is needed and created below. 
