...
```{r cluster}
# CSV file location of all pairwise comparisons of niche overlap in Schoener's D
...
for ( i in 1:31 ) {
  ...
  nclusters <- as.vector(unique(select(filter(partitions.df,domesticated),cluster)))
...
```{r omi}
omi.file <- sprintf('%s/results/OMI/standardized.averages.csv',REPO_HOME)
...
dom.taxa.clusters <- as.vector(unique(select(filter(partitions.df,domesticated),cluster)))$cluster
...
for ( i in 1:length(trait.names) ) {
  ...
  for ( j in 1:length(dom.taxa.clusters) ) {
    ...
    ingroup <- select(filter(partitions.df, cluster == c),taxon.name)
    outgroup <- select(filter(partitions.df, cluster != c),taxon.name)
...
mrcas <- list()
mrcas.labels <- c('a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z')
for ( i in 1:optimum.nclusters ) {
  tips <- select(filter(partitions.df,cluster==i),taxon.name)$taxon.name
  l <- mrcas.labels[i]
