...
for ( i in 1:length(taxa) ) {
  ...
  if ( file.exists(valid.model.prediction) ) {
    ...
    percentage.number <- round((0.1* nrow(points)), digits = 0)
    threshold.value <- sort.projection.values[percentage.number]
    species.pred[species.pred < species.pred] <- 0
...
for ( i in 1:length(taxa) ) {
  ...
  if (!file.exists(prediction.occurence.map.file)) {
    ...
    occ <- filter(occurrences.df, taxon_name == taxa[i])
...
```{r calc_dist}
# location of a triangular, inverse distance matrix file
overlap.file <- paste(REPO_HOME, "/results/maxent/overlap_SchoenersD.csv", sep = "")
