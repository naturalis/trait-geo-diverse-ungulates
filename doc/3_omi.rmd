...
normalized.values <- read.csv(normalized.repo, header=TRUE, row.names = 1)
...
overlap.csv <- paste(REPO_HOME, "/results/maxent/comparative/gower/maxent_omi.csv", sep="")
...
# write tree 
tree.file <- paste(REPO_HOME, "/results/maxent/comparative/gower/maxent_omi_hclust.tree", sep="")
write.tree(tree, file = tree.file)

tree.nj.file<- paste(REPO_HOME, "/results/maxent/comparative/gower/maxent_omi_nj.tree", sep="")
write.tree(nj.tree, file = tree.nj.file)
...
{
  clustering.pdf.file <- sprintf("%s/results/maxent/clustering_nj_raw_occurences.pdf", REPO_HOME)
